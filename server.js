...
// Middleware to parse JSON body - required for POST routes
...
app.post('/pix/pay', async (req, res) => {
        ...
        try {
            ...
            // valor pode ser centavos (number) ou real (string com decimal)
            const valorNum = parseFloat(valorReais || (valor / 100)) || 0;
            ...
            const result = await interRequest('POST', '/banking/v3/pix/pagamento', token, pixBody);
                    console.log('[pix/pay] Inter response status:', result.status);
