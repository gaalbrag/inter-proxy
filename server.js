...
// GET /diagnostics/boleto - probes candidate paths for boleto payment with an
// intentionally invalid linha digitavel, so Inter should reject it with a data
// error instead of processing a real payment. Use this to find the correct path.
app.get('/diagnostics/boleto', async (req, res) => {
        ...
        try {
                    ...
                    for (const p of boletoPaths) {
                                    ...
                                    results['POST ' + p] = { status: r.status, body: r.body.substring(0, 300) };
                    }
...
// The exact path has NOT been confirmed yet with a real call (same situation
// PIX was in before /diagnostics found the working path). Use
// GET /diagnostics/boleto to discover which one Inter actually accepts, then
// move that path to the front of this list.
...
// Expects: { linhaDigitavel | codigoBarras, valor (centavos) | valorReais, dataPagamento?, descricao? }
app.post('/boleto/pay', async (req, res) => {
        console.log('[boleto/pay] received body:', JSON.stringify(req.body));
        try {
                    const body = req.body || {};
                    const linhaDigitavel = (body.linhaDigitavel || body.codigoBarras || body.codBarraLinhaDigitavel || '').replace(/[.\s]/g, '');
                    const valorReais = body.valorReais;
                    const valor = body.valor;
                    const dataPagamento = body.dataPagamento || new Date().toISOString().split('T')[0];
                    const descricao = body.descricao || 'Pagamento de boleto';
            ...
            let parsed;
                    ...
                    if (result.status === 404) {
                                    parsed = {
                                                        error: 'Nenhum endpoint de pagamento de boleto respondeu (todos 404). Confirme o path correto na documentacao do Inter Developers e o escopo habilitado no app.',
